...
class CLIPLoss(nn.Module):
    ...
    def forward(self, out_ftir, out_raman, labels=None):
...
class ConstrastiveIdeaLoss(nn.Module):
    def __init__(self, init_tau=0.07):
        super().__init__()
        self.log_tau = nn.Parameter(torch.tensor(torch.log(torch.tensor(1.0 / init_tau))))
    ...
    def forward(self, out_ftir, out_raman, labels):
        ...
        logits_positive = logits * positive_mask
        logits_negative = logits * negative_mask

        mean_positive = logits_positive.sum(dim=1) / positive_mask.sum(dim=1)
        mean_negative = logits_negative.sum(dim=1) / negative_mask.sum(dim=1)

        loss = - torch.log(torch.exp(mean_positive) / (torch.exp(mean_positive) + torch.exp(mean_negative))).mean()
        return loss
